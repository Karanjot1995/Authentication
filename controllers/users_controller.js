      ...
      if(!user){
        ...
        ResetPasswordToken.findOne({
          ...
          ResetPasswordToken.create({
            ...
            if(err){console.log("error in creating token", err); return res.redirect('back');}
            console.log(token)
...
module.exports.setNewPassword = function(req,res){
  ResetPasswordToken.findOne({
    ...
    if(err){console.log("error in setting password",err);return res.redirect('/users/sign-in');}
    if(token){
      return res.render('reset_password', {title: "Reset Password", token:req.params.accesstoken})
    }else{
      return res.redirect('/users/sign-in')
...
module.exports.newPasswordSuccess = async function(req,res){
  try {
    ...
    console.log("error in setting password",error);return res.redirect('/users/sign-in');
  }
}